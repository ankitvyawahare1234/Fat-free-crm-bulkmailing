...
module ProgressBarHelper
  ...
  module Helper
    ...
    def static_progress_bar(name, value, display_percentage_text = false)
      ...
      result += image_tag('progress_bar_helper/large/percentImage.png',
        :style => "margin: 0pt; padding: 0pt; width: 120px; height: 12px;
                    background-position: #{(value*120-120).to_s}px 50%;
                    background-image: url(#{asset_path('progress_bar_helper/large/percentImage_back.png')});",
        :alt => (value*100).to_s+'%',
    ...
    def custom_static_progress_bar(name, value, options = {})
      ...
      if options[:bar_images].nil? then
        bar_image = "'"+image_path('progress_bar_helper/large/percentImage_back.png')+"'"
      else
        bar_image = "'"+image_path(options[:bar_images].first)+"'"
      end

      result =  '<span id="'+name+'_progress_bar" >'
      result += image_tag(options[:box_image] || 'progress_bar_helper/large/percentImage.png',
    ...
    def dynamic_progress_bar(name, value)
      value = ((value*100.0).round.to_f)/100.0
      return raw('<span class="progressBar" id="'+name+'">'+(value*100).to_s+'</span>')
    end
    ...
    def custom_dynamic_progress_bar(name, value, options = {})
      ...
      if options[:bar_images].nil? then
        bar_images = "'"+image_path('progress_bar_helper/large/percentImage_back.png')+"'"
      else
                                     ...
                                     (value*100).to_s+",
                                      {
                                              ...
                                              boxImage	: '"+image_path(options[:box_image] || 'progress_bar_helper/large/percentImage.png')+"',
                                              barImage	: "+bar_images+"
                                      }
                              );},
                        false);")
    ...
    def progress_bar(name,value, display_percentage_text = false, multicolor = false)
      ...
      if multicolor then
        result += custom_dynamic_progress_bar(name,value,{:show_text => display_percentage_text, :animate => true, :bar_images => [asset_path('progress_bar_helper/large/percentImage_red.png'), asset_path('progress_bar_helper/large/percentImage_back3.png'), asset_path('progress_bar_helper/large/percentImage_back2.png'), asset_path('progress_bar_helper/large/percentImage_back1.png')]})
      else
